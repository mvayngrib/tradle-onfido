...
function createClient (opts) {
  ...
  return {
    ...
    checkDocument: function (opts) {
      ...
      const { applicant, link, checkFace=true } = opts
      return changes.appendAsync({
    ...
    checkFace: co(function* (opts) {
      typeforce({
        applicant: typeforce.String,
        face: typeforce.String
      }, opts)

      const { applicant, face } = opts
      const { link } = yield db.getPendingDocument(applicant)
      return changes.appendAsync({
        ...
        switch (resource_type) {
          ...
          case 'check': {
            if (action === 'check.completed') {
              let id = object.id
              let doc = yield db.getDocumentByCheckId(id)
              entry = {
                topic: topics.documentstatus,
                link: doc.link,
                report: id,
