...
module.exports = function onfido (opts) {
  ...
  const docTopics = [
    topics.queuedocument,
    topics.document,
    topics.documentstatus,
    topics.queueface,
    topics.face
  ]
  ...
  const documents = indexer({
    ...
    primaryKey: 'link',
    filter: ({ topic }) => {
      return docTopics.indexOf(topic) !== -1
    },
    reduce: function (state, change, cb) {
      let newState
      const changeVal = change.value
      ...
      switch (topic) {
        ...
        case topics.queueface: {
          newState = documents.merge(state, change)
          newState.faceStatus = status.face.queued
          break
        }
        case topics.face: {
          newState = documents.merge(state, change)
          newState.faceStatus = status.face.uploaded
          break
        }
        default: {
          return cb()
  ...
  documents.on('change', function (change, newState, oldState) {
    ...
    case topics.face:
      return ee.emit('face:upload', newState)
    case topics.document:
  ...
  ;['status', 'check', 'report', 'result', 'id', 'faceStatus'].forEach(prop => {
  ...
  const getDocumentByCheckId = co(function* getDocumentByReportId (id) {
    typeforce(typeforce.String, id)

    const results = yield collect(indexes.documents.check.createReadStream({
      keys: false,
      eq: id
    }))
  ...
  function streamFacesToUpload (opts={}) {
    ...
    opts.eq = status.face.queued
    return indexes.documents.faceStatus.createReadStream(opts)
  }
